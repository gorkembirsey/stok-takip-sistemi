...
if not df_raw.empty:
    ...
    with st.sidebar.expander("🔄 Sütun Eşleştirme (Mapping)", expanded=True):
        # Akıllı Tahmin
        idx_loc = next(
            (i for i, c in enumerate(columns) if "loc" in c.lower() or "yer" in c.lower() or "depo" in c.lower()), 0)
        idx_qty = next((i for i, c in enumerate(columns) if
                        "qty" in c.lower() or "mik" in c.lower() or "adet" in c.lower() or "stok" in c.lower()),
                       1 if len(columns) > 1 else 0)
        idx_sku = next((i for i, c in enumerate(columns) if
                        "sku" in c.lower() or "kod" in c.lower() or "item" in c.lower() or "malzeme" in c.lower()),
                       2 if len(columns) > 2 else 0)
        idx_lot = next(
            (i for i, c in enumerate(columns) if "lot" in c.lower() or "batch" in c.lower() or "parti" in c.lower()), 0)
...
if not df.empty:
    ...
    with st.sidebar.expander("🔍 Akıllı Filtreler", expanded=True):
        ...
        secilen_yerler = st.multiselect("Lokasyon Seç:", tum_lokasyonlar, default=st.session_state['selected_locs'],
                                        label_visibility="collapsed", key='loc_multiselect')
        ...
        secilen_urunler = st.multiselect("Ürün Seç:", sorted(list(mevcut_urunler)), default=valid_defaults,
                                         label_visibility="collapsed", key='item_multiselect')
    ...
    if not df_filtered.empty:
        ...
        with tab1:
            ...
            k4.metric("🚨 Critical Items", f"{critical_count}", delta="-Action Needed" if critical_count > 0 else "OK",
                      delta_color="inverse")

            st.markdown("###")
            st.markdown(f"##### 📈 Top 15 Locations by Volume ({c_loc})")

            # --- DÜZELTME YAPILAN KISIM (BURASI GÜNCELLENDİ) ---
            # reset_index(name='Total_Quantity') diyerek isim çakışmasını engelliyoruz
            chart_data = df_filtered.groupby(c_loc)[c_qty].sum().reset_index(name='Total_Quantity').nlargest(15,
                                                                                                             'Total_Quantity')

            bar_chart = alt.Chart(chart_data).mark_bar(cornerRadius=5, color="#FFC107").encode(
                x=alt.X(c_loc, sort='-y', title=None),
                y=alt.Y('Total_Quantity', title='Quantity'),  # Yeni isim burada da kullanılıyor
                tooltip=[c_loc, 'Total_Quantity']
        ...
        with tab2:
            ...
            with col_sel:
                item_to_inspect = st.selectbox("🔍 Inspect Specific SKU details:",
                                               ["Select an SKU..."] + sorted(list(df_filtered[c_item].unique())))
            with col_btn:
                st.write("")
                st.write("")
    ...
    st.info("👋 Hoşgeldiniz. Lütfen Excel dosyanızı yükleyin.")